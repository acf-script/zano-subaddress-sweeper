...
**READ THE REST BEFORE STARTING CODE, SCRIPT DOES NOT WORK OUT THE BOX
**
...
I would not recommend editing the native_asset_id to other tokens such as FUSD I have not tested if this script works with tokens.
It just wouldn't by the way so don't try. I plan to eventually make it work for tokens on Zano so that's why the config is there but it's not going to work right now.
